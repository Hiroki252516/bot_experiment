...
class OllamaGenerationProvider(GenerationProvider):
    ...
    def extract_document_skill(
        ...
        prompt = (
            ...
            "教材本文に明示されている語句・英文・文法項目だけを使ってください。\n"
            "「主要な用語1」「概念X」「Term A」「ここに定義を書く」のようなプレースホルダーや一般論は禁止です。\n"
            "本文から読み取れない項目は空配列にし、推測で補完しないでください。\n"
            "本文が英語文法教材なら、仮定法、直説法、過去形、過去完了形、would/could/might、if節など本文固有の項目を抽出してください。\n"
    ...
    def _generate_adaptive_assessment(
        ...
        prompt = (
            ...
            "Document Agent Skill に含まれる具体的な文法項目・例文・説明だけを使ってください。\n"
            "概念X、A/B/Cだけの抽象問題、教材名だけを問う問題、本文にない一般論は禁止です。\n"
    ...
    def generate_learning_material(
        ...
        prompt = (
            ...
            "Document Agent Skill の具体語句・例文・文法項目を必ず本文に含め、抽象的な主要概念やプレースホルダーで説明しないでください。\n"
            "出力は必ずJSON schemaに従ってください。\n"
            f"cycle_index: {cycle_index}\n"
            f"Document Agent Skill: {json.dumps(document_skill, ensure_ascii=False)}\n"
